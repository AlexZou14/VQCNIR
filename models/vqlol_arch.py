...
import sys
from .vqlol_utils import ResBlock, CombineQuantBlock 
from .vgg_arch import VGGFeatureExtractor
from .main_decode import MainDecoder, HIMB, AIEM
...
class EnhanceLayers(nn.Module):
    def __init__(self, embed_dim=256, split_group=4, n_curve=3):
        super().__init__()
...
class MultiScaleEncoder(nn.Module):
    def __init__(self,
        ...
        if LQ_stage: 
            self.blocks.append(EnhanceLayers(**swin_opts))
...
class VQLOL(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
                 codebook_params=None, #[32, 1024, 512]
                 gt_resolution=256,
                 LQ_stage=False,
                 norm_type='gn',
                 act_type='silu',
        ...
        # build encoder 
        self.max_depth = int(np.log2(gt_resolution // self.codebook_scale[0]))  # log(256/32=8)=3
        encode_depth = int(np.log2(gt_resolution // self.scale_factor // self.codebook_scale[0]))  # 3
        ...
        self.main_decode = MainDecoder(base_channels=64, channel_multipliers=[1,2,4,4])
    ...
    def encode_and_decode(self, input, gt_indices=None, current_iter=None):
        ...
        for i in range(self.max_depth):
            ...
            if cur_res in self.codebook_scale:  # needs to perform quantize
                ...
                x = after_quant_feat
                dec_res_feats['z_quant'] = x

            x = self.decoder_group[i](x)
            dec_res_feats['Level_%d' % 2**(self.max_depth-i-1)]=x
            prev_dec_feat = x

        if self.LQ_stage:
            x = self.main_decode(dec_res_feats, enc_feats[0])
